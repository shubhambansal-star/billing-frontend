...
class PurchaseOrderListTab extends Component {
  ...
  assembleOptions = (list) => {
    const mandi_sales = list.filter(order=>{return order.party_type==="mandi-sales"})
    const sales = list.filter(order=>{return order.party_type==="sales"})
    ...
    const mandi_purchase_option = mandi_purchase.map(party=>{
      return{
        label: party.name,
        value: party.id,
    ...
    const mandi_sales_option = mandi_sales.map(party=>{
      return{
        label: party.name,
        value: party.id,
        val: party.party_type
    ...
    const sales_options = sales.map(party=>{
      return{
        label: party.name,
        value: party.id,
        val: party.party_type
    ...
    return [{label:"Mandi Purchase", options: mandi_purchase_option},{label:"Mandi Sales", options: mandi_sales_option},{label:"Purchase", options: purchase_options},{label:"Sales", options: sales_options},{label:"Broker", options: broker_options},{label:"Genes", options: GenesGroup}]
  }
            ...
            <Card elevation={6} className="invoice-details">
                <div className="row mt-4 ms-3 me-3 mb-2 justify-content-between">
                  <div className="col-md-3 mb-4">
                    <h4 className="fw-bold">Loading Unloading Register</h4>
                  </div>
